...
public final class NickServ implements Software, Module {
    ...
    private static final String DEFAULT_AUTH_SERVICE = "NickServ";
    ...
    // Track users that need to authenticate (numeric -> timestamp when they
    // connected)
    ...
    public void parseLine(String text) {
        ...
        try {
            ...
            if (jserv.getConfig().getConfigFile().getProperty("debug", "false").equalsIgnoreCase("true") &&
                ...
                LOG.log(Level.INFO,
                        "DEBUG NickServ parseLine: target={0}, expectedNumeric={1}, expectedNick@Host={2}@{3}",
            ...
            // Handle nick changes (N command with 3-4 parameters: source, N, newnick,
            // [timestamp])
            ...
            // For "BotName@Host", only check the nickname part before @ to support
            // different service hostnames
                ...
                if (isTargetingThisBot) {
                    if (jserv.getConfig().getConfigFile().getProperty("debug", "false").equalsIgnoreCase("true")) {
                        LOG.log(Level.FINE,
                                "DEBUG NickServ: Handling private message - target={0}, numeric={1}, nick={2}",
    ...
    private void handleNewUserDuringBurst(String[] elem) {
        ...
        // Users with PROTECT flag are excluded from nick protection enforcement
        int userFlags = jserv.getDb().getFlags(userNick);
        if (Userflags.hasFlag(userFlags, Userflags.Flag.PROTECT)) {
            LOG.log(Level.FINE, "User {0} has PROTECT flag, skipping nick protection",
                    userNumeric);
            return;
        }
        ...
        if (!isAuthenticatedForThisNick) {
            LOG.log(Level.INFO,
                    "User {0} ({1}) connected during burst with registered nick without matching authentication - killing immediately",
                    new Object[] { userNick, userNumeric });

            // Get user host for tracking (we need to reconstruct it from elem)
            // P10 N command format: server N nick hopcount timestamp ident host +modes
            // [base64ip] numeric :realname
            // Positions: [0] server [1] N [2] nick [3] hop [4] timestamp [5] ident [6] host
            // [7] modes
            String ident = elem[5];
            String host = elem[6];
     ...
     * 
     * P10 N command format: <server> N <nick> <hopcount> <timestamp> <ident> <host>
     * <modes> <base64ip> <numeric> :<realname>
     * Example: AB N WarPigs 1 1766478053 warpigs localhost +i B]AAAB ABAAA
     * :realname
     * 
     ...
     * elem[7] = user modes (+i, +r for authenticated, +k for service, +x for hidden
     * host, +o for oper)
     * elem[8] = base64 IP or account info if authenticated (B]AAAB or
     * account:timestamp:flags)
     * elem[9] = user numeric/token (ABAAA) - this is extracted by
     * extractNumericFromNCommand
        ...
        if (unauthenticatedUsers.containsKey(userNumeric)) {
            if (user != null) {
                ...
                if (accountName.equalsIgnoreCase(userNick) || isNickOwnedByAccount(userNick, accountName)) {
                    // Nick belongs to this account - remove from enforcement and send success
                    // notice
                    unauthenticatedUsers.remove(userNumeric);

                    LOG.log(Level.INFO,
                            "User {0} authenticated as {1} with matching nick {2}, removed from enforcement list",
                    ...
                    LOG.log(Level.INFO,
                            "User {0} authenticated as {1} but uses non-matching protected nick {2}, keeping in enforcement list",
                            new Object[] { userNumeric, accountName, userNick });
    ...
    private void handleNickChange(String[] elem) {
        ...
        if (!isNewNickRegistered) {
            ...
            if (unauthenticatedUsers.remove(userNumeric) != null) {
                LOG.log(Level.INFO, "User {0} removed from enforcement list (changed to unregistered nick)",
                        userNumeric);
            }
            return;
        }
        ...
        if (Userflags.hasFlag(userFlags, Userflags.Flag.PROTECT)) {
            LOG.log(Level.FINE, "Nick {0} has PROTECT flag, skipping nick protection",
                    newNick);
    ...
    /**
     * Handles LISTRESERVE command - lists all reserved nicknames for the user's
     * account
     */
    ...
    private void handleGhostCommand(String senderNumeric, String notice, Users user, String targetNick) {
        ...
        // Check if target is authenticated as same account (to prevent ghosting
    ...
    private void handleRecoverCommand(String senderNumeric, String notice, Users user, String targetNick) {
        ...
        if (targetNumeric != null && !targetNumeric.equals(senderNumeric)) {
            ...
            if (isDummy) {
                ...
                scheduleRecoverNickChange(senderNumeric, notice, targetNick, user, account, 800);
                return;
            }
        } else if (isDummy) {
        ...
        // Nick appears free - still use robust SN retry sequence
        scheduleRecoverNickChange(senderNumeric, notice, targetNick, user, account, 0);
    }

    private void scheduleRecoverNickChange(String senderNumeric, String notice, String targetNick,
            Users user, String account, long delayMs) {
        ...
        delayTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                Users currentUser = socketThread.getUsers().get(finalSenderNumeric);

                // User disconnected before nick change attempt.
                if (currentUser == null) {
                    return;
                }
                ...
                // Single service-driven forced nick change attempt (SN only).
                LOG.log(Level.INFO, "RECOVER nickchange single attempt (SN): {0} -> {1}",
                        new Object[] { finalSenderNumeric, targetNick });
    ...
    private void loadGLineConfiguration() {
        try {
            ...
            if (config != null) {
                ...
                LOG.log(Level.INFO,
                        "NickServ configuration loaded: grace_period={0}s, check_interval={1}ms, gline_enabled={2}, gline_attempts={3}, gline_duration={4}s, auth_service={5}",
                        new Object[] { gracePeriod, checkInterval, glineEnabled, glineAttempts, glineDuration,
                                authService });
            } else {
    ...
    /**
     * Checks if a user host should have a dummy nick created based on configured
     * attempts threshold
     */
            ...
            if (currentUser != null) {
                ...
                if (retryCount < 3) {
                    ...
                    if (!isAuthenticated) {
                        ...
                        LOG.log(Level.INFO, "Killing unauthenticated user {0} again (retry attempt)",
                                currentUserNumeric);
                            ...
                            for (Users u : socketThread.getUsers().values()) {
                                if (u.getNick().equalsIgnoreCase(nick)) {
                                    ...
                                    LOG.log(Level.SEVERE,
                                            "Nickname {0} still occupied after final kill - possible server issue",
                                            nick);
                                    break;
                                }
                            }
    ...
    private void createDummyNickFinal(String nick, String userHost) {
        try {
            ...
            // Format: <server> N <nick> <hopcount> <timestamp> <ident> <host> <modes>
            // <base64ip> <numeric> :<realname>
    ...
    /**
     * Checks all unauthenticated users and kills those that exceeded the grace
     * period
     */
            ...
            if (elapsed >= gracePeriod) {
                ...
                if (user != null) {
                    ...
                    int userFlags = jserv.getDb().getFlags(user.getNick());
                        if (Userflags.hasFlag(userFlags, Userflags.Flag.PROTECT)) {
                        LOG.log(Level.FINE,
                            "User {0} has PROTECT flag, skipping nick protection enforcement",
                                userNumeric);
                        unauthenticatedUsers.remove(userNumeric);
                        return;
                    }
    ...
    private void sendText(String text, Object... args) {
        ...
        String formatted = text.formatted(args);
        pw.println(formatted);
        pw.flush();
        if (jserv.getConfig().getConfigFile().getProperty("debug", "false").equalsIgnoreCase("true")) {
            System.out.println("DEBUG NickServ sendText: " + formatted);
            LOG.log(Level.FINE, "DEBUG sendText: {0}", formatted);
        }
    }

    /**
     * Extracts the user numeric/token from a P10 N command.
     * The user token is always the element directly before the ":" (realname
     * field).
     * 
     * P10 N command format: <server> N <nick> <hopcount> <timestamp> <ident> <host>
     * <modes> <base64ip> <numeric> :<realname>
     * Example: AB N WarPigs 1 1766478053 warpigs localhost +i B]AAAB ABAAA
     * :realname
     * In this example, ABAAA is the user token/numeric at position [9], right
     * before ":realname"
     * 
     * Important: User is authenticated if +r flag is present in modes (elem[7])
     * If +r is NOT present, the user is NOT authenticated regardless of the token
     * value.
     * 
